...
export class DataProvenanceValidationService {
  ...
  static validateSpaceTypeData(
    ...
    if (spaceType.provenance) {
      if (!this.validateProvenance(spaceType.provenance.rp)) reasons.push('Unverified Rp');
      if (!this.validateProvenance(spaceType.provenance.ra)) reasons.push('Unverified Ra');
      if (useDefaultOccupancy && !this.validateProvenance(spaceType.provenance.defaultOccupancy)) {
  ...
  static validateEzData(
    ...
    if (ezConfig.provenance) {
      if (!this.validateProvenance(ezConfig.provenance.ez)) reasons.push('Unverified Ez');
