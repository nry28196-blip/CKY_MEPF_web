...
const targetHeader = `<div className="flex items-center space-x-2">
            ...
            <h2 className="text-sm font-bold text-white uppercase tracking-wider">Friction Loss Reference Rates</h2>
          </div>
          <button 
            onClick={onClose}`;
...
const replaceHeader = `<div className="flex items-center space-x-2">
            <BookOpen className="w-5 h-5 text-sky-400" />
            <h2 className="text-sm font-bold text-white uppercase tracking-wider">{isKhmer ? 'ឯកសារយោងអត្រាកកិតបំពង់ខ្យល់' : 'Friction Loss Reference Rates'}</h2>
          </div>
